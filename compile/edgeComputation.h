...
__global__ void EdgeComputation_kernel(const int num_kernels, Dtype* input, Dtype* output, int height, int width) {
	...
	CUDA_KERNEL_LOOP(index, num_kernels)
	{
		...
		*(output + point_offset) = *(output + point_offset)/6;
		...
		// if (y-1 >= 0)
		// 	*(output + point_offset) += fabs(*(input + point_offset)-*(input + (y - 1) * width + x));
		// if (y+1 < height)
		// 	*(output + point_offset) += fabs(*(input + point_offset)-*(input + (y + 1) * width + x));
		// if (x-1 >= 0)
		// 	*(output + point_offset) += fabs(*(input + point_offset)-*(input + y * width + x - 1));
		// if (x+1 < width)
		// 	*(output + point_offset) += fabs(*(input + point_offset)-*(input + y * width + x + 1));
		// *(output + point_offset) = *(output + point_offset)/2;
	}
}
...
__global__ void EdgeComputation_backward_kernel(const int num_kernels, Dtype* input, Dtype* gradOutput, Dtype* gradInput, int height, int width) {
	...
	CUDA_KERNEL_LOOP(index, num_kernels)
	{
		...
		*(gradInput + point_offset) = *(gradInput + point_offset)/6;
		...
		// if (y-1 >= 0)
		// {
		// 	*(gradInput + point_offset) += (*(input + point_offset) > *(input + (y - 1) * width + x) ? 1 : -1) * *(gradOutput + point_offset);
		// 	*(gradInput + point_offset) += (*(input + point_offset) > *(input + (y - 1) * width + x) ? 1 : -1) * *(gradOutput + (y - 1) * width + x);
		// }
		// if (y+1 < height)
		// {
		// 	*(gradInput + point_offset) += (*(input + point_offset) > *(input + (y + 1) * width + x) ? 1 : -1) * *(gradOutput + point_offset);
		...
		// if (x-1 >= 0)
		// {
		// 	*(gradInput + point_offset) += (*(input + point_offset) > *(input + y * width + x - 1) ? 1 : -1) * *(gradOutput + point_offset);
		// 	*(gradInput + point_offset) += (*(input + point_offset) > *(input + y * width + x - 1) ? 1 : -1) * *(gradOutput + y * width + x - 1);
		// }
		// if (x+1 < width)
		// {
		// 	*(gradInput + point_offset) += (*(input + point_offset) > *(input + y * width + x + 1) ? 1 : -1) * *(gradOutput + point_offset);
		...
		// *(gradInput + point_offset) = *(gradInput + point_offset)/2;
	}
}
