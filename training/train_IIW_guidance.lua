...
local file = '/raid/qingnan/codes/qingnan_codes/train_IIW_guidance.lua'
  ...
  for i = 1,max_iters do
    ...
    if i <= 50 then
      ...
      if bestTestLoss > test_loss then
        ...
        model:clearState()
        torch.save(filename, model)
        local filename = string.format('%sstate_%s_best_joint.t7',netfiles,postfix)
        torch.save(filename, adam_state_save)      
      end
    end
