...
--GPU 0
model_guidance = torch.load('/mnt/codes/intrinsic/netfiles/model_IIW_guidance_best_joint.net')
model_intrinsic = torch.load('/mnt/codes/intrinsic/netfiles/model_joint_IIW_MPI_direct_best.net')

h0 = nn.Identity()()
r = h0 - model_intrinsic
g = h0 - model_guidance
R = {r,h0} - nn.JoinTable(2) - nn.IIWscale(1)
R_final = {R,g} - nn.JoinTable(2) - nn.DomainTransform(5,0.25,30,0)

model = nn.gModule({h0},{r,g,R_final})
model = model:cuda()

criterion = nn.ParallelCriterion():add(nn.WHDRHingeLossPara(0.12,0.08,1),1):add(nn.MSECriterion(),0.35):add(nn.WHDRHingeLossPara(0.12,0.08,1),0.1)
criterion = criterion:cuda()

criterion_test1 = nn.WHDRHingeLossPara(0.1,0,1)
...
local file = '/mnt/codes/qingnan_codes/train_joint_IIW_MPI_combine.lua'
